...
class GetOptionPropertiesUseCase @Inject constructor(
    private val iCategoriesRepository: ICategoriesRepository
) {

    suspend operator fun invoke(optionId: Int): DataResult<List<PropertyModel?>?> {

        return when (val response = iCategoriesRepository.getOptionProperties(optionId)) {
